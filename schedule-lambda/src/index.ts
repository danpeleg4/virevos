...
async function deleteSchedule(scheduleName: string): Promise<void> {
...
async function handleScheduledEmail(scheduledEmailId: number): Promise<void> {
  const rows = await db
    .select()
  ...
  if (scheduledEmail.status !== "pending") {
    console.log("Email already processed, skipping:", scheduledEmailId);
    return;
  }
  ...
  try {
    ...
    if (scheduledEmail.awsScheduleName) {
      await deleteSchedule(scheduledEmail.awsScheduleName);
    }
...
export const handler = async (event: LambdaEvent): Promise<void> => {
  ...
  console.log(meetingEvent.userId);
  try {
    await db
      .update(events)
      .set({ status: "active" })
      .where(eq(events.id, meetingEvent.id));
  } catch (error) {
    console.error(error);
  }
};
